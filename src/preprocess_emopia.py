...
EMOPIA_DIR = PROJECT_ROOT / "res" / "emopia_data" / "EMOPIA_2.2"
MIDI_DIR = EMOPIA_DIR / "midis"
LABEL_FILE = EMOPIA_DIR / "label.csv"
OUTPUT_DIR = PROJECT_ROOT / "res" / "emopia_tokens"
SAMPLE_RATE = 32000
...
SF2_PATH = PROJECT_ROOT / "res" / "GeneralUser-GS.sf2"
FLUIDSYNTH_BIN = os.environ.get("FLUIDSYNTH_BIN", "fluidsynth")
USE_FLUIDSYNTH = SF2_PATH.exists() and shutil.which(FLUIDSYNTH_BIN) is not None

if USE_FLUIDSYNTH:
    print(f"使用 FluidSynth 高质量合成: {FLUIDSYNTH_BIN}")
else:
    print("降级为 pretty_midi 合成（未检测到 fluidsynth 或 sf2）")
